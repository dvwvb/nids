...
/* Active cycle */ 
function rollingActive02 (activeList02) {
	$(activeList02).each(function  (index) {
		$itemList02 = $(this);
		$item02 = $itemList02.find("li");
		itemLength02 = $item02.length;
		startNum02 = 0;
		rollingSpeed02 = $itemList02.data("rolling-time");
		
		function visualTime02(){
			if(startNum02 < ( itemLength02 - 1)){
				startNum02++;
			}else{
				startNum02 = 0;
			}
		...
		function visualPlay02(){
			$item02.each(function(id){
				if(id == startNum02){
					$(this).addClass("active"); // li에 클래스 붙이기
				}else{
					$(this).removeClass("active");
		...
		visualPlay02();
		visual_timer02 = setInterval(visualTime02,rollingSpeed02);
	});
}
