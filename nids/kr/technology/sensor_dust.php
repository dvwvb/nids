<!doctype html>
<html lang="ko">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>먼지센싱기술 - Technology - 엔아이디에스</title>
    <meta name="Title" content="먼지센싱기술 - Technology - 엔아이디에스">
    <meta name="Subject" content="먼지센싱기술">
    <meta name="Keywords" content="">
    <meta name="Description" content="엔아이디에스">
    <meta property="og:type" content="website">
    <meta property="og:title" content="먼지센싱기술 - Technology - 엔아이디에스">
    <meta property="og:description" content="엔아이디에스">
    <meta property="og:image" content="../../images/common/meta_guide.png">
    <meta property="og:url" content="sensor_dust.html">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:title" content="먼지센싱기술 - Technology - 엔아이디에스">
    <meta name="twitter:description" content="엔아이디에스">
    <meta name="twitter:image" content="../../../images/common/meta_guide.png">
    <meta name="twitter:domain" content="sensor_dust.html">
    <meta name="google-site-verification" content="zNv1c93lJs_H39Y9Opss1FG3KgBpQ-hFNVa9E3uHQuA">
    <meta name="naver-site-verification" content="a9d5de59ece03f218947004eae31d8bd1c1a7e46">
    <meta name="viewport" content="width=device-width, initial-scale=1"><!-- 모바일사이트, 반응형사이트 제작시 사용 -->
    <meta name="format-detection" content="telephone=no" /><!-- ios 자동전화걸기 방지 -->
    <meta name="NaverBot" content="All" />
    <meta name="NaverBot" content="INDEX,FOLLOW" />
    <meta name="Yeti" content="All" />
    <meta name="Yeti" content="INDEX,FOLLOW" />
    <meta name="Googlebot" content="All" />
    <meta name="Googlebot" content="INDEX,FOLLOW" />
    <meta name="Robots" content="INDEX,FOLLOW">
    <meta name="theme-color" content="#222222">
    <link rel="shortcut icon" href="https://www.nids.co.kr/nids/favicon.ico">
    <link rel="canonical" href="https://www.nids.co.kr/">
    <link rel="stylesheet" href="../../../css/default.css">
    <link rel="stylesheet" href="../../css/layout07c8.css?ver=220914">
    <link rel="stylesheet" href="../../css/contentdb8d.css?ver=22114">
    <link rel="stylesheet" href="../../css/content_mj07c8.css?ver=220914">
    <link rel="stylesheet" href="../../css/maindbfb.css?ver=220921" />
    <link rel="stylesheet" href="../css/language.css" /><!-- 해당언어에서만 사용하는 css -->
    <link rel="stylesheet" href="../../css/layout_responsivea8b3.css?ver=220826">
    <link rel="stylesheet" href="../../css/content_responsive0948.css?ver=220829">
    <link rel="stylesheet" href="../../css/main_responsive7f06.css?ver=220908_1" />
    <link rel="stylesheet" href="../../../css/animate.css" />
    <link rel="stylesheet" type="text/css" href="../../../../cdn.jsdelivr.net/npm/xeicon%402.3.3/xeicon.min.css"><!-- XEicon -->
    <!-- Board Skin -->
    <link rel="stylesheet" href="../../../css/common/cm_bbs_common54e3.css?ver=220908">
    <link rel="stylesheet" href="../../../css/common/cm_board.css">
    <link rel="stylesheet" href="../../../css/common/cm_certification.css">
    <link rel="stylesheet" href="../../../css/common/cm_gallery.css" />
    <link rel="stylesheet" href="../../../css/common/cm_sitemap.css" />
    <!-- Plugin -->
    <link rel="stylesheet" type="text/css" href="../../../css/plugin/slick.css">
    <link rel="stylesheet" type="text/css" href="../../../css/plugin/jquery.mCustomScrollbar.css">
    <link rel="stylesheet" type="text/css" href="../../../css/plugin/magnific-popup.css">
    <link rel="stylesheet" type="text/css" href="../../../css/plugin/swiper.css" />
    <link rel="stylesheet" type="text/css" href="../../../css/plugin/spotlight.css" />

    <script type="text/javascript" src="../../../js/vendor/jquery-3.6.0.min.js"></script>
    <style>
    </style>

    <script>
        //  2번째 대메뉴 인덱스
        dep1 = 2

        // dep1 = 02,
        dep2 = 01;

        // sub2_num 변수가 있을때 ( 3차메뉴 )
        dep3 = 01;
        //console.log(dep3);
    </script>
</head>

<body>
    <!-- accessibility -->
    <div class="cm-accessibility">
        <a href="#container">본문바로가기</a>
    </div>
    <!-- //accessibility -->
    <!-- code -->
    <div id="wrap" class="sub-wrap   business-wrap">
        <!-- header -->

        <header id="header" class="top-fixed-object">
            <!-- .top-fixed가 붙는 object에 top-fixed-object 클래스 추가  -->
            <div class="gnb-overlay-bg"></div>
            <div id="headerInnerWrap">
                <!-- ****************** 헤더상단 ********************** -->
                <div id="headerInner" class="area-wide">
                    <h1 class="logo">
                        <a href="../index-2.html" title="to home">
                            <svg id="Layer_2" class="logo-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 395.24 162.94">
                                <g id="Layer_1-2">
                                    <g>
                                        <path class="cls-3" d="M0,30.1s57.07,12.93,76.04,57.83C76.04,87.93,141.64-.92,225.89,0c0,0,34.25,10.64,70.35,7.41,0,0-68.97,13.42-90.72-.46,0,0-83.32,34.72-130.99,117.11,0,0-25.46-71.75-74.53-93.97Z" />
                                        <g>
                                            <path class="cls-2" d="M102.54,40.2h23.64l30.85,45.34V40.2h23.87V122.14h-23.87l-30.69-44.99v44.99h-23.81V40.2Z" />
                                            <path class="cls-2" d="M198.73,40.2h25.38V122.14h-25.38V40.2Z" />
                                            <path class="cls-2" d="M242.61,40.2h37.62c7.41,0,13.41,1.01,17.97,3.02,4.56,2.01,8.33,4.9,11.32,8.66,2.98,3.76,5.14,8.14,6.48,13.13,1.34,5,2.01,10.29,2.01,15.87,0,8.76-1,15.55-2.99,20.37-1.99,4.82-4.76,8.87-8.3,12.13-3.54,3.26-7.34,5.43-11.4,6.51-5.55,1.49-10.58,2.24-15.09,2.24h-37.62V40.2Zm25.32,18.56v44.77h6.2c5.29,0,9.06-.59,11.29-1.76,2.24-1.17,3.99-3.22,5.25-6.15,1.26-2.92,1.9-7.67,1.9-14.23,0-8.68-1.42-14.62-4.25-17.83-2.83-3.21-7.53-4.81-14.08-4.81h-6.32Z" />
                                            <path class="cls-2" d="M320.9,95.03l24.09-1.51c.52,3.91,1.58,6.89,3.19,8.94,2.61,3.32,6.34,4.97,11.18,4.97,3.61,0,6.4-.85,8.36-2.54s2.93-3.66,2.93-5.9-.93-4.03-2.79-5.7c-1.86-1.68-6.19-3.26-12.97-4.75-11.11-2.5-19.02-5.81-23.76-9.95-4.77-4.14-7.15-9.41-7.15-15.82,0-4.21,1.22-8.19,3.66-11.93,2.44-3.74,6.11-6.69,11.01-8.83s11.62-3.21,20.15-3.21c10.47,0,18.45,1.95,23.95,5.84,5.5,3.89,8.77,10.09,9.81,18.59l-23.87,1.4c-.63-3.69-1.96-6.37-4-8.05-2.03-1.68-4.84-2.52-8.41-2.52-2.95,0-5.16,.62-6.65,1.87s-2.24,2.77-2.24,4.56c0,1.31,.61,2.48,1.84,3.52,1.19,1.08,4.02,2.09,8.5,3.02,11.07,2.38,19,4.8,23.78,7.24,4.79,2.44,8.27,5.47,10.45,9.08,2.18,3.62,3.27,7.66,3.27,12.13,0,5.26-1.45,10.1-4.36,14.53-2.91,4.44-6.97,7.8-12.19,10.09-5.22,2.29-11.79,3.44-19.73,3.44-13.94,0-23.59-2.68-28.95-8.05-5.37-5.37-8.4-12.19-9.11-20.46Z" />
                                        </g>
                                    </g><text class="cls-1" transform="translate(101.54 152.01)">
                                        <tspan class="cls-4" x="0" y="0">Sensor </tspan>
                                        <tspan class="cls-5" x="116.68" y="0">T</tspan>
                                        <tspan class="cls-6" x="133.72" y="0">echnology</tspan>
                                    </text>
                                </g>
                            </svg>
                        </a>
                    </h1>
                    <div class="header-util-box">
                        <!-- 외국어선택 -->
                        <div class="header-lang-list">
                            <ul class="clearfix">
                                <li class="cur"><a href="../index-2.html">KOR</a></li>
                                <li><a href="https://www.nids.co.kr/nids/en/">ENG</a></li>
                                <li><a href="https://www.nids.co.kr/nids/cn/">CHN</a></li>
                                <li><a href="https://www.nids.co.kr/nids/es/">ESP</a></li>
                            </ul>
                        </div>
                        <!-- 사이트맵 -->
                        <button class="sitemap-line-btn cm-sitemap-open-btn" title="Open the sitemap"><i class="xi-drag-vertical"></i></button>
                    </div>
                </div>
                <!-- ****************** GNB ********************** -->
                <!-- GNB PC  -->
                <nav id="gnb" class="each-menu">
                    <h2 class="blind">Menu</h2>
                    <!-- 
					- 전체메뉴 : class="total-menu"
					- 각각메뉴 : class="each-menu" + <div id="gnbBg"></div> 삭제
				-->
                    <!-- <div id="gnbBg"></div> -->
                    <ul class="clearfix area-box">
                        <li class="gnb1">
                            <a href="../business/sensor_dust.html">사업분야</a>
                            <div class="gnb-2dep">
                                <ul>
                                    <li>
                                        <a href="../business/sensor_dust.html"><span>환경센서</span></a>
                                        <ul class="gnb-3dep">
                                            <li><a href="../business/sensor_dust.html"><span>먼지센서</span></a></li>
                                            <li><a href="../business/sensor_gas.html"><span>가스센서</span></a></li>
                                        </ul>
                                    </li>
                                    <li>
                                        <a href="../business/plasma.html"><span>공기살균 & 탈취</span></a>
                                        <ul class="gnb-3dep">
                                            <li><a href="../business/plasma.html"><span>플라즈마 모듈</span></a></li>
                                            <li><a href="../business/photocatalytic.html"><span>광촉매 모듈</span></a></li>
                                            <li><a href="../business/hepa.html"><span>향균헤파필터</span></a></li>
                                        </ul>
                                    </li>
                                    <li>
                                        <a href="../business/iot.html"><span>IoT 시스템</span></a>
                                    </li>
                                </ul>
                            </div>
                        </li>
                        <li class="gnb2">
                            <a href="sensor_dust.html">기술소개</a>
                            <div class="gnb-2dep">
                                <ul>
                                    <li>
                                        <a href="sensor_dust.html"><span>센서기술</span></a>
                                        <ul class="gnb-3dep">
                                            <li><a href="sensor_dust.html"><span>먼지센싱기술</span></a></li>
                                            <li><a href="sensor_gas.html"><span>가스센싱기술</span></a></li>
                                        </ul>
                                    </li>
                                    <li>
                                        <a href="plasma.html"><span>공기살균 & 탈취 기술</span></a>
                                        <ul class="gnb-3dep">
                                            <li><a href="plasma.html"><span>플라즈마 기술</span></a></li>
                                            <li><a href="photocatalytic.html"><span>광촉매 기술</span></a></li>
                                            <li><a href="hepa.html"><span>향균헤파필터</span></a></li>
                                        </ul>
                                    </li>
                                </ul>
                            </div>
                        </li>
                        <li class="gnb3">
                            <a href="../company/intro.html">회사소개</a>
                            <div class="gnb-2dep">
                                <ul>
                                    <li><a href="../company/intro.html"><span>회사개요</span></a></li>
                                    <li><a href="../company/greeting.html"><span>CEO 인사말</span></a></li>
                                    <li><a href="../company/news.html"><span>뉴스</span></a></li>
                                </ul>
                            </div>
                        </li>
                        <li class="gnb4">
                            <a href="../customer/online.html">고객센터</a>
                            <div class="gnb-2dep">
                                <ul>
                                    <li><a href="../customer/online.html"><span>Contact us</span></a></li>
                                    <li><a href="../customer/faq.html"><span>FAQ</span></a></li>
                                    <li><a href="../customer/recruit.html"><span>인재채용</span></a></li>
                                </ul>
                            </div>
                        </li>
                        <li class="gnb5">
                            <a href="https://www.nids.co.kr/airvom/kr/index.php">에어봄 <i class="xi-external-link"></i></a>
                        </li>
                    </ul>
                </nav>
            </div>
            <!-- GNB Mobile -->
            <button class="nav-open-btn" title="Menu Open"><i class="xi-drag-vertical"></i></button>
            <div class="gnb-overlay-bg-m"></div>
            <nav id="gnbM" class="gnb-style-basic">
                <!-- 
				기본스타일 : .gnb-style-basic
				Full 스타일 : .gnb-style-full
			-->
                <h2 class="blind">Menu</h2>
                <div class="header-util-menu-box">
                    <!-- 언어 선택 리스트 -->
                    <ul class="clearfix lang-select">
                        <li class="cur"><a href="../index-2.html">KOR</a></li>
                        <li><a href="javascript: alert('준비중입니다.');">ENG</a></li>
                        <li><a href="javascript: alert('준비중입니다.');">CHN</a></li>
                        <li><a href="javascript: alert('준비중입니다.');">ESP</a></li>
                    </ul>
                    <!-- // -->
                </div>
                <div class="gnb-navigation-wrapper">
                    <div class="gnb-navigation-inner">
                        <ul id="navigation" data-menu-clone="true"></ul><!-- PC메뉴 같을때 true / 같지않으면 false 후 메뉴삽입 -->
                    </div>
                </div>
            </nav>
        </header> <!-- //header -->
        <!-- container -->
        <div id="container">
            <!-- visual -->
            <!-- 서브비주얼 없을때 visual_type ="none" -->
            <section id="visual" class="sub-visual-technology">
                <div class="visual-img-con">
                    <div class="visual-img-inner" style="background:#eee url(../../images/layout/sub_visual_technology.jpg) no-repeat 70% 0%"></div>
                </div>
                <div class="area visual-txt-con">
                    <div class="table-cell-layout">
                        <div class="visual-txt-container">
                            <h2 class="visual-tit trans400">
                                Technology </h2>
                            <p class="visual-sub-txt">With Aivom’s Plasma Technology, <br class="m-br">Eliminate</p>
                        </div>
                    </div>
                </div>
            </section>
            <!-- //visual -->
            <!-- middleArea -->
            <div id="middleArea">
                <!-- ****************** 서브메뉴 ********************** -->

                <!-- 2차&3차 같이 있는 타입 -->
                <!-- 서브메뉴1 -->
                <aside id="topBusinessMenu" class="">
                    <div class="side-menu-inner">
                        <div class="area">
                            <aside id="topMenu04" class="">
                                <div class="side-menu-inner">
                                    <div class="cm-top-menu clearfix">
                                        <div class="menu-location location2 cm-drop-menu-box-JS" data-drop-event="click">
                                            <button class="cur-location cm-drop-open-btn-JS">
                                                <span>센서기술</span>
                                                <i class="xi-caret-down-circle-o arrow"></i>
                                            </button>
                                            <ul class="location-menu-con cm-drop-list-JS">
                                                <li>
                                                    <a href="sensor_dust.html"><span>센서기술</span></a>
                                                    <ul class="gnb-3dep">
                                                        <li><a href="sensor_dust.html"><span>먼지센싱기술</span></a></li>
                                                        <li><a href="sensor_gas.html"><span>가스센싱기술</span></a></li>
                                                    </ul>
                                                </li>
                                                <li>
                                                    <a href="plasma.html"><span>공기살균 & 탈취 기술</span></a>
                                                    <ul class="gnb-3dep">
                                                        <li><a href="plasma.html"><span>플라즈마 기술</span></a></li>
                                                        <li><a href="photocatalytic.html"><span>광촉매 기술</span></a></li>
                                                        <li><a href="hepa.html"><span>향균헤파필터</span></a></li>
                                                    </ul>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            </aside>
                            <ul class="business-3dep-list sub-menu-technology clearfix">
                                <li class="swiper-slide gnb-name"><a href="sensor_dust.html"><span>먼지센싱기술</span></a></li>
                                <li class="swiper-slide gnb-name"><a href="sensor_gas.html"><span>가스센싱기술</span></a></li>
                            </ul>
                        </div>
                    </div>
                </aside>
                <!-- // -->

                <!-- 모바일 서브메뉴 2 -->
                <aside id="topMenuM02" class="dep2-menu cm-top-menu clearfix">
                    <div class="side-menu-inner clearfix">
                        <!-- 2차메뉴 -->
                        <div class="menu-location location2 cm-drop-menu-box-JS" data-drop-event="click">
                            <button class="cur-location cm-drop-open-btn-JS">
                                <span>센서기술</span>
                                <i class="xi-caret-down-circle-o arrow"></i>
                            </button>
                            <ul class="location-menu-con cm-drop-list-JS">
                                <li>
                                    <a href="sensor_dust.html"><span>센서기술</span></a>
                                    <ul class="gnb-3dep">
                                        <li><a href="sensor_dust.html"><span>먼지센싱기술</span></a></li>
                                        <li><a href="sensor_gas.html"><span>가스센싱기술</span></a></li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="plasma.html"><span>공기살균 & 탈취 기술</span></a>
                                    <ul class="gnb-3dep">
                                        <li><a href="plasma.html"><span>플라즈마 기술</span></a></li>
                                        <li><a href="photocatalytic.html"><span>광촉매 기술</span></a></li>
                                        <li><a href="hepa.html"><span>향균헤파필터</span></a></li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
                    </div>
                </aside>
                <aside id="topMenuM02" class="cm-top-menu clearfix">
                    <div class="side-menu-inner clearfix">
                        <!-- 3차메뉴 -->
                        <div class="menu-location location3 cm-drop-menu-box-JS" data-drop-event="click">
                            <button class="cur-location cm-drop-open-btn-JS">
                                <span>먼지센싱기술</span>
                                <i class="xi-caret-down-circle-o arrow"></i>
                            </button>
                            <ul class="location-menu-con cm-drop-list-JS">
                                <li class="swiper-slide gnb-name"><a href="sensor_dust.html"><span>먼지센싱기술</span></a></li>
                                <li class="swiper-slide gnb-name"><a href="sensor_gas.html"><span>가스센싱기술</span></a></li>
                            </ul>
                        </div>
                    </div>
                </aside>
                <!-- // -->


                <!-- content -->
                <section id="content" class="wide ">

                    <section class="sub-page technology-page">
                        <h3 class="cm-page-tit" data-scroll="fade-up">먼지센싱기술</h3>
                        <!-- 상단 영상 -->
                        <article class="sub-cm-visual-video technology-visual-video" data-scroll="fade-up">
                            <article class="background-video-wrapper">
                                <div class="background-video">
                                    <div id="player" data-code="APqn9Cffntg"></div> <!-- 유투브 코드값 -->
                                </div>
                            </article>
                            <div class="visual-line-box">
                                <span class="line"></span>
                            </div>
                            <div class="txt-con" data-scroll="fade-up">
                                <div class="txt-con-box area">
                                    <div class="txt-inner">
                                        <p class="txt02">Strong Precision and Reliability of <br />NIDS Dust Sensor Technology</p>
                                        <div class="sub-cm-btn-box">
                                            <a href="#" class="sub-cm-btn style03"><i class="xi-search"></i> <span>자세히보기</span></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </article>
                        <!-- // -->
                        <article class="technology-sensor-con01 ">
                            <div class="area">
                                <div class="sensor-item-wrap" data-scroll="fade-up">
                                    <div class="left-box">
                                        <h4 class="tit">Reliable Sensor Technology</h4>
                                        <div class="info-box sensor-rolling-JS" data-rolling-time="4000">
                                            <dl class="selected">
                                                <dt>제조 표준으로 제품 신뢰성 확보</dt>
                                                <dd>
                                                    엔아이디에스 센서 반도체 제조 표준을 바탕으로 안정적인 대량 생산과 <br class="pc-br" />
                                                    소형화 작업을 활성화하였습니다. 덕분에 시스템 비용 및 크기 절감에 성공하며, <br class="pc-br" />
                                                    표준화된 보정 시스템으로 제조시간을 단축할 수 있습니다.
                                                </dd>
                                            </dl>
                                            <dl>
                                                <dt>측정 정밀도 향상으로 정확도 보장</dt>
                                                <dd>
                                                    엔아이디에스는 민감한 아날로그 센서 신호 현장의 증폭 및 디지털 간섭에서 벗어나, <br class="pc-br" />
                                                    강하고 정밀도를 향상 시킨 센서측정 기술을 제공합니다. 또한, 고장이 발생하기 쉬운 부분에서의 <br class="pc-br" />
                                                    꾸준한 개선 연구와 분석기술의 개발로 사용자에게 장기 안정성 혜택을 누릴 수 있도록 합니다.
                                                </dd>
                                            </dl>
                                            <dl>
                                                <dt>소형화 센서 기술 </dt>
                                                <dd>
                                                    마이크로 센서 구조는 표적화된 마이크로시스템 기술 처리 단계를 통해 개발되었습니다. <br />
                                                    특히, 특허 받은 반도체 칩 생산 기술은 센서 아날로그 및 <br class="pc-br" />
                                                    디지털 평가 전자 장치의 융합을 가능하게 합니다.
                                                </dd>
                                            </dl>
                                        </div>
                                    </div>
                                    <div class="right-box">
                                        <div class="img-list">
                                            <div class="img"><img src="../../images/content/tecnnology_dust_con01_img01.jpg" alt=""></div>
                                            <div class="img"><img src="../../images/content/tecnnology_dust_con01_img02.jpg" alt=""></div>
                                            <div class="img"><img src="../../images/content/tecnnology_dust_con01_img03.jpg" alt=""></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </article>

                        <article class="technology-sensor-con02">
                            <div class="area-box">
                                <div class="technology-tit-box" data-scroll="fade-up">
                                    <h4 class="technology-tit">엔아이디에스 <br class="m-br" />먼지센서 기술</h4>
                                    <p class="technology-txt">엔아이디에스의 독창적 기술을 포함한 <br class="m-br" />기술리포트 모음집을 확인하세요.</p>
                                    <a href="#" class="technology-btn"><i class="xi-pen"></i> Contact Us</a>
                                </div>

                                <div class="sensor-item" data-scroll="fade-up">
                                    <div class="img-box">
                                        <span><img src="../../images/content/tecnnology_dust_con02_img01.png" alt="Laser Particle Sensor"></span>
                                    </div>
                                    <div class="txt-box">
                                        <h4 class="tit">Laser Particle Sensor</h4>
                                        <p class="caption"><i class="xi-error"></i> 제품의 사용 전 주의사항과 작동방법을 꼭 확인하세요.</p>
                                        <div class="btn-list">
                                            <a href="#" class="inner-btn" download>SM-LS-01C <i class="xi-download"></i></a>
                                            <a href="#" class="inner-btn" download>SM-LS-03A <i class="xi-download"></i></a>
                                            <a href="#" class="inner-btn" download>SM-LS-05B <i class="xi-download"></i></a>
                                            <a href="#" class="inner-btn" download>NLS006 <i class="xi-download"></i></a>
                                        </div>
                                    </div>
                                </div>
                                <div class="sensor-item" data-scroll="fade-up">
                                    <div class="img-box">
                                        <span><img src="../../images/content/tecnnology_dust_con02_img02.png" alt="IR Dust Sensor"></span>
                                    </div>
                                    <div class="txt-box">
                                        <h4 class="tit">IR Dust Sensor</h4>
                                        <p class="caption"><i class="xi-error"></i> 제품의 일반적인 스펙과 기술적 특성을 확인하세요.</p>
                                        <div class="btn-list">
                                            <a href="#" class="inner-btn" download>SM-PWM-01A <i class="xi-download"></i></a>
                                            <a href="#" class="inner-btn" download>SM-PWM-03A <i class="xi-download"></i></a>
                                            <a href="#" class="inner-btn" download>SM-PWM-03A-G <i class="xi-download"></i></a>
                                            <a href="#" class="inner-btn" download>SM-ND-10A <i class="xi-download"></i></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </article>

                        <article class="technology-sensor-con03">
                            <div class="area">
                                <div class="tec-con03-item">
                                    <div data-scroll="fade-up">
                                        <div class="technology-tit-box">
                                            <h4 class="technology-tit">IR DUST SENSOR 측정원리</h4>
                                            <p class="technology-txt">
                                                IR Dust Sensor의 작동 원리는 광학산란 방식입니다. <br />
                                                공기는 센서의 흡기 밸브를 통해 들어가며, 반복적으로 전환되는 적외선 LED 조명은 공기 중 먼지 입자를 센싱 합니다.
                                            </p>
                                        </div>
                                        <div class="check-list">
                                            <p class="check-txt">신뢰성 정밀도 측정</p>
                                            <p class="check-txt">높은 장기 안정성</p>
                                        </div>
                                    </div>

                                    <div class="video-iframe-wrapper" data-scroll="fade-up">
                                        <div class="video-iframe-box"></div>
                                        <div class="video-cover-box" data-code="QdUsqFLMX20">
                                            <!-- 유투브ID -->
                                            <span class="thumb"><img src="../../../../img.youtube.com/vi/QdUsqFLMX20/0.jpg" alt="" /></span>
                                            <!-- <div class="icon"><i class="xi-play-circle-o"></i></div> -->
                                        </div>
                                    </div>

                                    <div class="info-box" data-scroll="fade-up">
                                        <div class="left-box">
                                            <h4 class="tit">High accuracy IR dust Sensor</h4>
                                            <strong class="sub-tit">20년 이상의 기술 혁신성</strong>
                                            <p class="txt">
                                                엔아이디에스 엔지니어들은 20년 이상에 걸친 독창적 기술력으로 수많은 <br />
                                                까다로운 시장과 애플리케이션에 적합한 독창적인 먼지센서 기술을 개발하였습니다. <br />
                                                이 기술력은 광학 부품에 먼지가 쌓이는 것을 방지하고 장기적 사용성에 대한 <br />
                                                안정성을 제공합니다.
                                            </p>
                                        </div>
                                        <div class="right-box">
                                            <span><img src="../../images/content/technology_dust_con03_img01.jpg" alt="High accuracy IR dust Sensor"></span>
                                        </div>
                                    </div>
                                </div>
                                <div class="tec-con03-item">
                                    <div class="technology-tit-box" data-scroll="fade-up">
                                        <h4 class="technology-tit">Laser Particle SENSOR 측정원리</h4>
                                        <p class="technology-txt">
                                            센서가 출력하는 펄스 신호는 입자에 레이저가 산란된 후 디지털 신호로 변환됩니다. <br />신호 파형과 입자 직경 사이의 상관관계에 기초한 분석으로 입자의 수와 직경을 센싱 합니다.
                                        </p>
                                    </div>

                                    <div class="video-iframe-wrapper" data-scroll="fade-up">
                                        <div class="video-iframe-box"></div>
                                        <div class="video-cover-box" data-code="1dyb6yS7XnQ">
                                            <!-- 유투브ID -->
                                            <span class="thumb"><img src="../../../../img.youtube.com/vi/1dyb6yS7XnQ/0.jpg" alt="" /></span>
                                            <!-- <div class="icon"><i class="xi-play-circle-o"></i></div> -->
                                        </div>
                                    </div>

                                    <div class="info-box">
                                        <div class="left-box">
                                            <h4 class="tit">High Accuracy Laser Particle Sensor</h4>
                                            <p class="txt">
                                                엔아이디에스의 독자적인 알고리즘으로 먼지 입자의 종류 관계없이
                                                크기 등급을 결정할 수 있는 새로운 개념을 적용하였습니다. <br />
                                                또한, 질량 농도의 측정을 보다 개선 및 향상시켜 에어로졸을 정확하게
                                                구별 가능하게 하고 광범위한 환경 조건에서도 정밀하게 측정할 수
                                                있도록 센싱 신뢰도를 높였습니다.
                                            </p>
                                        </div>
                                        <div class="right-box">
                                            <span class="pc"><img src="../../images/content/technology_dust_con03_img02.jpg" alt="High Accuracy Laser Particle Sensor"></span>
                                            <span class="mobile">
                                                <img src="../../images/content/technology_dust_con03_img02_m01.jpg" alt="High Accuracy Laser Particle Sensor">
                                                <img src="../../images/content/technology_dust_con03_img02_m02.jpg" alt="High Accuracy Laser Particle Sensor">
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </article>



                        <article class="technology-sensor-banner banner01">
                            <div class="area">
                                <h4 class="tit">Improving Indoor Air Quality</h4>
                                <p class="txt">
                                    현시대 사람들은 80-90%의 시간을 밀폐된 실내 공간에서 보내고 있습니다. <br />
                                    대기 중 수많은 미세 오염물질은 눈의 자극이나, 두통, 피로감 등의 증상을 야기 시키며, 사람들의 건강문제에 직접적인 영향을 미치고 있습니다. <br />
                                    엔아이디에스는 실내 공기의 질을 측정하고 적절범위 내 조절하는 것이 웰빙과 건강을 직접적 연결 짓는 핵심 열쇠로 여겨, <br />
                                    귀사 제품에 채용 시 최대효과기능을 발휘하도록 돕고 있습니다.
                                </p>
                            </div>
                        </article>

                        <article class="sensor-con-04">
                            <div class="area">
                                <div class="tit-box" data-scroll="fade-up">
                                    <strong class="sub-tit">How to improve air quality?</strong>
                                    <h4 class="sensor-tit">환기와 정화는 깨끗한 공기를 유지하는 최선의 선택 방안이 될 것입니다.</h4>
                                    <p class="sensor-txt">
                                        우리 센서를 활용한 실내 공기 질 측정 데이터 값은 공기 정화 시스템을 구성하거나 <br class="pc-br" />
                                        환기 시스템을 현명하게 관리하는데 활용되는 귀사의 핵심 기술력이 될 것입니다.
                                    </p>
                                </div>

                                <div class="sensor-con-04-list">
                                    <div class="list-item left-item" data-scroll="fade-up">
                                        <div class="img-box">
                                            <span><img src="../../images/content/technology_dust_con04_img01.jpg" alt="At home"></span>
                                        </div>
                                        <div class="txt-box">
                                            <h4 class="tit">At home</h4>
                                            <p class="txt">
                                                실외 공기 오염은 열린 창문이나 환기 시스템을 통해 실내로 이동합니다.
                                                특히, 그을음, 자동차 배기가스, 꽃가루와 같은 실외 오염 물질로 노출된 실내환경은 깨끗한
                                                공기 질 확보를 위해 관리 되어야할 항목으로 입자 센싱의 중요성은 점차 증대되었습니다.
                                                페인트, 직물, 청소 제품 그리고 요리 냄새나 먼지 진드기 등 실내 환경 악화요소에 대한
                                                종합적 실내 공기 질 관리는 가정 환경에서 적극 고려되어야 할 기술입니다.
                                            </p>
                                            <div class="btn-box">
                                                <a href="#" class="btn-style01">Air Conditioner</a>
                                                <a href="#" class="btn-style02">Related Sensor</a>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="list-item right-item" data-scroll="fade-up">
                                        <div class="img-box">
                                            <span><img src="../../images/content/technology_dust_con04_img02.jpg" alt="HVAC"></span>
                                        </div>
                                        <div class="txt-box">
                                            <h4 class="tit">HVAC</h4>
                                            <p class="txt">
                                                현 스마트건물이 보급되며, 실내 공기의 질을 감시하는 시스템은 빠르게 설치되어
                                                가동되고 있습니다. 원격 컨트롤에서부터 무선 통신 및 API를 제공하는 정보는
                                                난방, 환기 및 에어컨(HVAC)을 처리하는 전체 시스템에서 채용되어지고 있습니다.
                                            </p>
                                            <div class="btn-box">
                                                <a href="#" class="btn-style01">Air Conditioner</a>
                                                <a href="#" class="btn-style02">Related Sensor</a>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="list-item left-item" data-scroll="fade-up">
                                        <div class="img-box">
                                            <span><img src="../../images/content/technology_dust_con04_img03.jpg" alt="At the Workplace"></span>
                                        </div>
                                        <div class="txt-box">
                                            <h4 class="tit">At the Workplace</h4>
                                            <p class="txt">
                                                COVID-19 팬데믹 상황에서의 집단감염 사례는 직원들의 건강과 직접적인 연관성을
                                                보여주는 작업장 공기 질 관리개선 요구의 실 사례입니다. 일반적으로 사무실의 환기율을
                                                높여 실내 공기 질 개선을 기대할 수 있으나, 실내구조 제한 및 대기 질 사내규정 적용의
                                                미비는 프린터에서 나오는 유해물질 또는 실외 대기오염물질로부터 직원들을 쉽게
                                                노출되게 합니다. 특히, 습도와 온도 외에도, CO2, 휘발성 유기화합물, 미세 입자와 같은
                                                대기 오염 물질은 사무실의 업무 수행과 복지에 영향을 미칠 수 있습니다. 직장내 실내 공기
                                                질 관리 기술 도입에 따른 사무실 업무환경의 개선도모는 직원 간 의사 결정, 그리고 적은
                                                결근의 결과로 이어져 높은 생산성을 도모합니다.
                                            </p>
                                            <div class="btn-box">
                                                <a href="#" class="btn-style01">Air Conditioner</a>
                                                <a href="#" class="btn-style02">Related Sensor</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </article>

                        <article class="technology-sensor-banner banner02">
                            <div class="area">
                                <h4 class="tit">Dangers in indoor air</h4>
                                <p class="txt">
                                    밀폐 실내 공간 내 발생하는 먼지, 곰팡이, 세균, 화학물질과 같은 대기오염물질은 밀폐된 공간에 축적되어, <br class="pc-br" />
                                    우리 삶과 밀접 관련 있는 가정과 사무실의 공기 질의 부정적인 영향을 미쳐 건강상의 위험신호를 야기 시키고 있습니다.
                                </p>
                            </div>
                        </article>

                        <article class="technology-sensor-con04">
                            <div class="area">
                                <div class="tec-con04-list">
                                    <div class="tec-con04-item">
                                        <span class="icon"><img src="../../images/content/technology_sensor_bottom_icon01.png" alt=""></span>
                                        <dl>
                                            <dt>이산화탄소</dt>
                                            <dd>
                                                이산화탄소는 폐쇄된 공간에서는 인간의 활동 및 <br />
                                                점유와 상관관계를 갖습니다. 특히, 높은 농도는 <br />
                                                두통, 졸음, 무기력, 성능 저하를 일으킬 수 있어 <br />
                                                센서를 통한 실내농도 확인 및 준수기준이 권장됩니다.
                                            </dd>
                                        </dl>
                                    </div>
                                    <div class="tec-con04-item">
                                        <span class="icon"><img src="../../images/content/technology_sensor_bottom_icon02.png" alt=""></span>
                                        <dl>
                                            <dt>이산화질소</dt>
                                            <dd>
                                                이산화질소는 공장, 자동차 매연 등 화석연료의 <br />
                                                연소에 의해 생성되며 VOCs와 반응하여 오존을 <br />
                                                일으키는 전구물질 역할을 합니다. 특히, 이산화질소의 <br />
                                                과다 노출은 인체에 부정적 영향을 미치고 폐 기능에 <br />
                                                부정적인 영향을 미칠 수 있습니다.
                                            </dd>
                                        </dl>
                                    </div>
                                    <div class="tec-con04-item">
                                        <span class="icon"><img src="../../images/content/technology_sensor_bottom_icon03.png" alt=""></span>
                                        <dl>
                                            <dt>포름알데히드</dt>
                                            <dd>
                                                포름알데히드는 목재 기반 재료, 바닥재 또는 페인트에서 <br />
                                                접착 성분 등 공산품 생산에 다양하게 쓰이나 새집증후군의 <br />
                                                주된 원인으로 평가됩니다. 특히, 50ppm 이상의 높은 농도에서의
                                                노출은 눈 자극이나 심지어 암을 유발할 수 있습니다.
                                            </dd>
                                        </dl>
                                    </div>
                                    <div class="tec-con04-item">
                                        <span class="icon"><img src="../../images/content/technology_sensor_bottom_icon04.png" alt=""></span>
                                        <dl>
                                            <dt>휘발성 유기 화합물</dt>
                                            <dd>
                                                휘발성 유기화합물은 대기 중 탄소를 함유한 물질로 <br />
                                                단기 노출은 자극, 현기증, 천식 악화로 이어질 수 있고, <br />
                                                장기간 노출 시 폐암이나 간, 신장, 신경계 손상을 <br />
                                                일으킬 수 있습니다.
                                            </dd>
                                        </dl>
                                    </div>
                                    <div class="tec-con04-item">
                                        <span class="icon"><img src="../../images/content/technology_sensor_bottom_icon05.png" alt=""></span>
                                        <dl>
                                            <dt>미세먼지</dt>
                                            <dd>
                                                미세먼지는 자연배출원 뿐만 아니라 여러 종류의 <br />
                                                고정배출원 또는 이동배출원으로부터 발생되어 <br />
                                                천식과 같은 호흡기계 질병을 악화 시키고 <br />
                                                알레르기 반응을 유발 시킵니다. <br />
                                                따라서, 실내외 모두 적극적 관리지침의 마련에 <br />
                                                따른 정밀 측정계 활용이 적극 요구됩니다.
                                            </dd>
                                        </dl>
                                    </div>
                                    <div class="tec-con04-item">
                                        <span class="icon"><img src="../../images/content/technology_sensor_bottom_icon06.png" alt=""></span>
                                        <dl>
                                            <dt>온도 및 습도</dt>
                                            <dd>
                                                온습도의 컨트롤은 인간이 살아가는 최적 환경을 컨트롤 <br />
                                                하는데 중요한 요소입니다. 인체는 40-60%의 습도 범위에서 <br />
                                                가장 편안함을 느끼는 반면, 건조한 공기가 호흡기를 자극하고 <br />
                                                습한 공기는 응결로 이어져 곰팡이 침입의 방아쇠가 <br />
                                                될 수 있습니다.
                                            </dd>
                                        </dl>
                                    </div>
                                </div>
                            </div>
                        </article>
                    </section>
                </section>
                <!-- //content -->
            </div>
            <!-- //middleArea -->
        </div>
        <!-- //container -->
        <!-- footer -->
        <footer id="footer">
            <button class="to-top-btn"><em class="blind">TOP</em><i class="xi-arrow-up"></i></button>
            <!-- 푸터 상단 -->
            <article id="footerInner">
                <div class="area-box clearfix">
                    <!-- 왼쪽 -->
                    <div class="footer-left-con">
                        <div class="footer-address-info-box">
                            <h5 class="footer-address-info-tit" data-scroll="fade">고객지원 </h5>
                            <div class="footer-address-list" data-scroll="fade">
                                <h5>본사 / 엔아이디에스 <a href="#" class="place-icon"><i class="xi-maker"></i></a></h5>
                                <dl class="col1">
                                    <dt>주소</dt>
                                    <dd>경기도 성남시 중원구 갈마치로 314 717호 (우)13201</dd>
                                </dl>
                                <dl>
                                    <dt>Tel</dt>
                                    <dd>+82-31-742-8200</dd>
                                </dl>
                                <dl class="col2">
                                    <dt>E-mail</dt>
                                    <dd><a class="mail" href="mailto:sales@nids.co.kr">sales@nids.co.kr</a></dd>
                                </dl>
                            </div>
                            <div class="footer-address-list" data-scroll="fade">
                                <h5>브랜드 / 에어봄<a href="#" class="place-icon"><i class="xi-maker"></i></a></h5>
                                <dl class="col1">
                                    <dt>주소</dt>
                                    <dd>경기도 성남시 중원구 갈마치로 314 B2층 15호 (우)13201</dd>
                                </dl>
                                <dl>
                                    <dt>Tel</dt>
                                    <dd>+82-31-745-0207</dd>
                                </dl>
                                <dl class="col2">
                                    <dt>E-mail</dt>
                                    <dd><a class="mail" href="mailto:support@airvom.com">support@airvom.com</a></dd>
                                </dl>
                            </div>
                        </div>
                    </div>
                    <!-- 오른쪽 -->
                    <article class="footer-right-con" data-scroll="fade" data-scroll-delay="500">
                        <ul class="foot-menu clearfix">
                            <li><a href="../etc/privacy.html" class="cm-modal-open-btn"><b>PRIVACY POLICY</b></a></li>
                        </ul>
                        <span class="foot-logo">
                            <svg id="Layer_2" class="foot-logo-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 395.24 162.94">
                                <g id="Layer_1-2">
                                    <g>
                                        <path class="cls-3" d="M0,30.1s57.07,12.93,76.04,57.83C76.04,87.93,141.64-.92,225.89,0c0,0,34.25,10.64,70.35,7.41,0,0-68.97,13.42-90.72-.46,0,0-83.32,34.72-130.99,117.11,0,0-25.46-71.75-74.53-93.97Z" />
                                        <g>
                                            <path class="cls-2" d="M102.54,40.2h23.64l30.85,45.34V40.2h23.87V122.14h-23.87l-30.69-44.99v44.99h-23.81V40.2Z" />
                                            <path class="cls-2" d="M198.73,40.2h25.38V122.14h-25.38V40.2Z" />
                                            <path class="cls-2" d="M242.61,40.2h37.62c7.41,0,13.41,1.01,17.97,3.02,4.56,2.01,8.33,4.9,11.32,8.66,2.98,3.76,5.14,8.14,6.48,13.13,1.34,5,2.01,10.29,2.01,15.87,0,8.76-1,15.55-2.99,20.37-1.99,4.82-4.76,8.87-8.3,12.13-3.54,3.26-7.34,5.43-11.4,6.51-5.55,1.49-10.58,2.24-15.09,2.24h-37.62V40.2Zm25.32,18.56v44.77h6.2c5.29,0,9.06-.59,11.29-1.76,2.24-1.17,3.99-3.22,5.25-6.15,1.26-2.92,1.9-7.67,1.9-14.23,0-8.68-1.42-14.62-4.25-17.83-2.83-3.21-7.53-4.81-14.08-4.81h-6.32Z" />
                                            <path class="cls-2" d="M320.9,95.03l24.09-1.51c.52,3.91,1.58,6.89,3.19,8.94,2.61,3.32,6.34,4.97,11.18,4.97,3.61,0,6.4-.85,8.36-2.54s2.93-3.66,2.93-5.9-.93-4.03-2.79-5.7c-1.86-1.68-6.19-3.26-12.97-4.75-11.11-2.5-19.02-5.81-23.76-9.95-4.77-4.14-7.15-9.41-7.15-15.82,0-4.21,1.22-8.19,3.66-11.93,2.44-3.74,6.11-6.69,11.01-8.83s11.62-3.21,20.15-3.21c10.47,0,18.45,1.95,23.95,5.84,5.5,3.89,8.77,10.09,9.81,18.59l-23.87,1.4c-.63-3.69-1.96-6.37-4-8.05-2.03-1.68-4.84-2.52-8.41-2.52-2.95,0-5.16,.62-6.65,1.87s-2.24,2.77-2.24,4.56c0,1.31,.61,2.48,1.84,3.52,1.19,1.08,4.02,2.09,8.5,3.02,11.07,2.38,19,4.8,23.78,7.24,4.79,2.44,8.27,5.47,10.45,9.08,2.18,3.62,3.27,7.66,3.27,12.13,0,5.26-1.45,10.1-4.36,14.53-2.91,4.44-6.97,7.8-12.19,10.09-5.22,2.29-11.79,3.44-19.73,3.44-13.94,0-23.59-2.68-28.95-8.05-5.37-5.37-8.4-12.19-9.11-20.46Z" />
                                        </g>
                                    </g><text class="cls-1" transform="translate(101.54 152.01)">
                                        <tspan class="cls-4" x="0" y="0">Sensor </tspan>
                                        <tspan class="cls-5" x="116.68" y="0">T</tspan>
                                        <tspan class="cls-6" x="133.72" y="0">echnology</tspan>
                                    </text>
                                </g>
                            </svg>
                        </span>
                        <div class="foot-sns-menu">
                            <ul class="clearfix">
                                <li><a href="#" target="_blank" title="새창으로열기"><i class="xi-youtube"></i></a></li>
                                <li><a href="#" target="_blank" title="새창으로열기"><i class="xi-facebook"></i></a></li>
                                <li><a href="#" target="_blank" title="새창으로열기"><i class="xi-twitter"></i></a></li>
                            </ul>
                        </div>
                        <div class="footer-copyright">COPYRIGHT(C) 2022. NIDS CO.,LTD. ALL RIGHTS RESERVED.</div>
                    </article>
                </div>
            </article>
        </footer>
        <article id="footerPartnerListWrap" data-scroll>
            <article id="footerPartnerListContent01">
                <div class="area-box" dir="rtl">
                    <ul class="footer-partner-list footer-partner-list01 clearfix">
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner01_img01.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner01_img02.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner01_img03.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner01_img04.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner01_img05.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner01_img06.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner01_img07.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner01_img08.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner01_img01.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner01_img02.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                    </ul>
                </div>
            </article>
            <article id="footerPartnerListContent02">
                <div class="area-box">
                    <ul class="footer-partner-list footer-partner-list02 clearfix">
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner02_img01.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner02_img02.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner02_img03.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner02_img04.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner02_img05.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner02_img06.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner02_img07.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner02_img01.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner02_img02.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                        <li class="partner-item">
                            <div class="partner-item-inner">
                                <span><img src="../../images/common/footer_partner02_img03.jpg" alt="파트너사 이름" /></span>
                            </div>
                        </li>
                    </ul>
                </div>
            </article>
        </article>

        <div class="mouse-pointer">
            <div class="pointer-circle"></div>
            <span class="pointer-txt">DRAG</span>
        </div> <!-- //footer -->
        <section id="siteMapCon03" class="cm-sitemap-wrapper">
            <div class="cm-sitemap-container cm-sitemap-container-03">
                <div class="sitemap-bg cm-sitemap-bg">
                    <!-- logo 배경 있을경우 -->
                    <!-- <div class="logo-bg"><span>LOGO</span></div> -->
                </div>
                <div class="cm-sitemap-inner-con">
                    <section class="cm-sitemap-area-con area">
                        <div class="sitemap-modal-tit-con-03 cm-modal-tit-con">
                            <h1 class="cm-modal-tit" data-splitting>SITEMAP</h1>
                            <button title="Close Sitemap" class="sitemap-close-btn cm-sitemap-close-btn"><i class="xi-close-thin"></i></button>
                        </div>
                        <article class="sitemap-wrapper-style sitemap-wrapper-style03"></article>
                    </section>
                </div>
            </div>
        </section>
    </div>
    <!-- //wrap -->
    <script type="text/javascript" src="../../../js/vendor/jquery.easing.1.3.js"></script>
    <script type="text/javascript" src="../../../js/plugin/slick.js"></script>
    <script type="text/javascript" src="../../../js/plugin/swiper.min.js"></script>
    <script type="text/javascript" src="../../../js/plugin/gsap.min.js"></script>
    <script type="text/javascript" src="../../../js/plugin/ScrollToPlugin.min.js"></script>
    <script type="text/javascript" src="../../../js/plugin/splitting.min.js"></script>
    <script type="text/javascript" src="../../../js/plugin/skrollr.min.js"></script>
    <script type="text/javascript" src="../../../js/plugin/jquery.waypoints.min.js"></script>
    <script type="text/javascript" src="../../../js/plugin/jquery.mCustomScrollbar.concat.min.js"></script>
    <script type="text/javascript" src="../../../js/plugin/jquery.magnific-popup.js"></script>
    <script type="text/javascript" src="../../../js/plugin/spotlight.min.js"></script>
    <script type="text/javascript" src="../../../js/functions54e3.js?ver=220908"></script>
    <script type="text/javascript" src="../../js/navb66a.js?ver=220701"></script>
    <script type="text/javascript" src="../../../js/layer_popup.js"></script>
    <script type="text/javascript" src="../../js/common07c8.js?ver=220914"></script>
    <script type="text/javascript" src="../../js/subdbfb.js?ver=220921"></script>
    <script type="text/javascript">
        $(document).ready(function() {
            /* ***********************  영상 클릭시 ************************ */
            $(".video-cover-box").on("click", function() {
                var videoCode = $(this).data("code");
                $(this).fadeOut().prev(".video-iframe-box").append('<iframe width="100%" height="100%" src="https://www.youtube.com/embed/' + videoCode + '?version=3&showinfo=0&autoplay=1&loop=1&controls=1&modestbranding=1&rel=0&playlist=' + videoCode + '" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>');
            });
        });


        rollingActiveSensor(".sensor-rolling-JS");

        /* ************************
         * 순차적으로 active클래스가 붙는 함수
         * @param activeList : 선택자
         ************************ */
        /* Active cycle */
        function rollingActiveSensor(activeList) {
            $(activeList).each(function(index) {
                $itemList = $(this);
                $item = $itemList.find("dl");
                itemLength = $item.length;
                startNum = 0;
                rollingSpeed = $itemList.data("rolling-time");

                function visualTime() {
                    if (startNum < (itemLength - 1)) {
                        startNum++;
                    } else {
                        startNum = 0;
                    }
                    visualPlay();
                }

                function visualClick() {
                    $item.on("click", function() {
                        if (!$(this).hasClass('active')) {
                            $item.children('dd').hide();
                            $item.removeClass("active");
                            $(this).children('dd').show();
                            $(this).addClass("active");
                        }
                    })
                }

                function visualPlay() {
                    $item.each(function(id) {
                        if (id == startNum) {
                            $(this).addClass("active"); // li에 클래스 붙이기
                            $(this).children('dd').show();
                        } else {
                            $(this).removeClass("active");
                            $(this).children('dd').hide();
                        }
                    });
                };
                visualPlay();
                visualClick();

                visual_timer = setInterval(visualTime, rollingSpeed);

                $item.mouseover(function() {
                    clearInterval(visual_timer);
                })
                $item.mouseout(function() {
                    visual_timer = setInterval(visualTime, rollingSpeed);
                })

            });
        }
    </script>

</body>

</html>