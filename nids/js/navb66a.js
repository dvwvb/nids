...
var $snb3depMenu = $("#topMenu02");
var $snbBusiness3depMenu = $(".business-3dep-list");
// PC
var $gnb = $("#gnb");
...
function openEachMenu () {
	$gnbItem.children("a").on("mouseenter focus",function  () {
		...
		$gnb_dep2_item.on("mouseenter focus",function  () {
			$(this).next(".gnb-3dep").addClass("open");
		});
		$gnb_dep2.children("ul").children("li").on("mouseleave",function  () {
...
function activeMenuIdx () {
	if ( dep1 > 0 && dep2 > 0) {
		...
		if ( dep3 > 0 ) {
			$snb3depMenu.find("li").eq(dep3-1).addClass("on");
			$snbBusiness3depMenu.find("li").eq(dep3-1).addClass("on");
		}
